...
**Goodhart's Law (Goodhart, 1975; Strathern, 1997; Manheim & Garrabrant, 2018).** "When a measure becomes a target, it ceases to be a good measure." Goodhart warns against optimizing a proxy, which destroys the proxy's validity. This argument is less vulnerable to Goodhart gaming than RLHF by structural design: it targets signal *fidelity* (one layer of indirection from behavior) rather than a learned proxy (two layers). The framework delays Goodhart vulnerability to meta-level — the model would need to learn to corrupt its own calibration instruments while appearing to preserve them, not merely optimize a proxy. This is a harder failure mode to fall into and harder to exploit, but remains theoretically possible. The distinction is structural — maximizing S is Goodhart-vulnerable; preserving S's calibration is structurally harder to game because the target is the instrument's accuracy, not its reading, but the possibility of meta-level gaming is not eliminated. Manheim & Garrabrant's taxonomy of four Goodhart variants (regressional, extremal, causal, adversarial) addresses proxy gaming. The three corruptions identified here (conversational, agentic, paternalistic sycophancy) address signal distortion — a related but distinct failure taxonomy.
